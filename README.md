...
> 🏆 **Статус проекта:** NESTREN Protocol одобрен экспертным советом **Сколково** и получил грант на дальнейшее развитие R&D и пилотные внедрения в критической сетевой инфраструктуре.
>
> 💼 **Грантовый фокус (2026–2028):** создание production-версии post-quantum VPN платформы, сертификация, интеграции с enterprise/гос-сегментом и международное масштабирование.
...
## 📌 Roadmap: Сколково и внедрение

### Этап 1 — Grant R&D (Q3 2026 – Q2 2027)
- Интеграция реальных PQ-библиотек (ML-KEM/ML-DSA через audited backend).
- Формальная верификация state machine handshake и security review.
- Production anti-replay bitmap window + hardened record layer.
- Подготовка к пилотам: observability, policy engine, централизованный key lifecycle.

### Этап 2 — Пилотные внедрения (Q3 2027 – Q1 2028)
- Пилоты в финтехе, промышленном IoT и распределённых edge-сетях.
- Интеграция с корпоративными IAM/PKI и Zero-Trust NAC контурами.
- SLA/SLO модель для high-load эксплуатации и blue/green rekey rollout.
- Поддержка аппаратных HSM и защищённых контуров управления ключами.

### Этап 3 — Масштабирование и рынок (Q2 2028+)
- Вывод NESTREN Secure Access Platform как managed/self-hosted решения.
- Внедрение в провайдерские backbone/VPN-overlays и мультиоблачные среды.
- Экспортный сценарий: OEM-партнёрства с телеком/кибербезопасностью.
- Расширенный профиль совместимости: QUIC-like transport mode, FIPS/ГОСТ build targets.

### Предполагаемая судьба проекта
NESTREN развивается как технологическая платформа для защищённой связности нового поколения: от защищённых каналов между дата-центрами и филиалами до межкластерного шифрования в cloud-native инфраструктуре и промышленных сетях. Проект ориентирован на практическое внедрение в сегментах, где одновременно критичны post-quantum устойчивость, Zero-Trust модель и высокая пропускная способность.
